...
### `"clustermq"`, `"clustermq_staged"`, and `"hasty"`
...
In hasty mode, `drake` <span style = "color: red;">abandons its [claims to reproducibility](https://ropensci.github.io/drake/#evidence)</span> and focuses exclusively on job scheduling. It <span style = "color: red;">forgets about tracking output</span>, and it <span style = "color: red;">no longer tries to skip up-to-date targets</span>. In fact, it runs every target every time you call `make()`. But in exchange, job scheduling is much faster for projects with large datasets and lots of targets.

We retain an important advantage in hasty mode: `drake` still knows which targets to run in parallel and which need to wait for dependencies to finish. In other words, hasty mode is still smart enough to traverse end-to-end pipelines of jobs that need to wait for each other.

```{r hastydag, echo = FALSE, message = FALSE}
...
<span style = "color: red;">**But we emphasize: in hasty mode, the [CORE REPRODUCIBILITY CLAIMS of `drake`](https://ropensci.github.io/drake/#evidence) are NO LONGER VALID. Here, `drake` cannot provide evidence that your results are synchronized with the underlying code and data.** Hasty mode should only be used for debugging and testing or if computional reproducibility is handled some other way.</span>

In addition, the return values of targets remain available in memory while `make()` is executing, but those values are no longer stored in a cache. Consequently,

1. You need to save all your important results manually to your own `file_out()` files.
2. `loadd()` and `readd()` can no longer retrieve data.
3. Subsequent non-hasty calls to `make()`s start from scratch.

Lastly, `drake` no longer tries to limit the memory it uses, so you should avoid returning giant objects from commands in your plan.
