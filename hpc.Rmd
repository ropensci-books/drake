...
**But we emphasize: in blitz mode, the [CORE REPRODUCIBILITY CLAIMS of `drake`](https://ropensci.github.io/drake/#evidence) are NO LONGER VALID. Here, `drake` cannot provide evidence that your results are synchronized with the underlying code and data.** Blitz mode should only be used

1. For debugging and testing.
2. If computational reproducibility is adequately handled some other way.
3. If the data are large and the targets are numerous and/or easy to compute by comparison. In those cases, normal usage of `drake` and may not be useful ([example](http://inf.ufrgs.br/gppd/wsppd/2018/papers/proceedings/WSPPD_2018_paper_10.pdf)).
