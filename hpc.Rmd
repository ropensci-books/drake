...
Most clusters will let you submit `make()` as a job on a compute node. Let's consider the Sun Grid Engine (SGE) as an example. First, we create a script that calls `make()` (or [`r_make()`](https://ropensci.github.io/drake/reference/r_make.html)).
