...
It is possible to supply a custom job scheduler function to the `parallelism` argument of `make()`. Functions `backend_hasty()` and `backend_future_lapply_staged()` from packages [`drake.hasty`](https://github.com/wlandau/drake.hasty) and [`drake.future.lapply.staged`](https://github.com/wlandau/drake.future.lapply.staged), respectively, are examples. You might consider writing your own such function if you wish to
