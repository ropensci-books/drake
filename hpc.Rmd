...
```{r blitzdag, echo = FALSE, message = FALSE}
...
<span style = "color: red;">**But we emphasize: in blitz mode, the [CORE REPRODUCIBILITY CLAIMS of `drake`](https://ropensci.github.io/drake/#evidence) are NO LONGER VALID. Here, `drake` cannot provide evidence that your results are synchronized with the underlying code and data.** Blitz mode should only be used for debugging and testing or if computional reproducibility is handled some other way.</span>
