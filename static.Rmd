...
### Unnested grouping variables

The forward propagation of grouping variables is powerful, but it requires each grouping variable to be strictly nested inside a *single* group of targets (e.g. a *single* call to `map()` or `cross()`). The following plan disrupts this nesting, so `drake` does not know how to generate `c1`.
...
But if you reinstantiate grouping variables `a1`, `a2`, and `a3` just for `c1`, then `drake` produces the expected `c1`.
...
drake_plan(
  ...
  b2 = target(1, transform = map(a1 = 1, a3 = 1, .id = FALSE)),
  b3 = target(1, transform = map(a2 = 1, a3 = 1, .id = FALSE)),
  c1 = target(1, transform = map(a1 = 1, a2 = 1, a3 = 1, .id = FALSE)),
  trace = TRUE
)
```

Other workarounds include `bind_plans()` (on separate sub-plans) and [dynamic branching](#dynamic). Regardless, there is no substitute for checking and visualizing plans as described earlier.
