...
Each grouping variable must be defined only once. In the plan below, there are multiple conflicting definitions of `a1`, `a2`, and `a3`, and `drake` does not know which one to use.
...
Other workarounds include `bind_plans()` (on separate sub-plans) and [dynamic branching](#dynamic).Always check your plans (`vis_drake_graph()` etc.) before you run them.
