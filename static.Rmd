...
Targets can reuse grouping variables defined in dependencies upstream, and this forward propagation is powerful. However, it can easily go wrong when the same grouping variable is defined in multiple places. In the plan below, there are multiple conflicting definitions of `a1`, `a2`, and `a3`, and `drake` does not know which one to use.
...
Workarounds include `bind_plans()` (on separate sub-plans) and [dynamic branching](#dynamic). Regardless, always check your plans before you run them.
