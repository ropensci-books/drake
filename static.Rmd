...
*Grouping variables* are the custom arguments to `map()` and `cross()` that govern which targets to create. To better understand how grouping variables work in your plan, set the `trace` argument of `drake_plan()` to `TRUE`. That way, the plan gets special columns to keep track of the `tuning_setting`, `mean_value`, and `analysis` associated with each target.
