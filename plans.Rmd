...
A `drake` plan is a data frame with columns named `target` and `command`. Each target an R object produced in your workflow, and each command is the R code to produce it. You can create plans any way you like, and the `drake_plan()` function is particularly convenient. Recall the plan from our [previous example](#main):
...
`drake_plan()` does not run the workflow, it only creates the plan. To build the actual targets, we need to run `make()`. Creating the plan is like writing an R script, and running `make(your_plan)` is like calling [`source("your_script.R")`](https://www.rdocumentation.org/packages/base/versions/3.5.2/topics/source) an R script.
...
If you have ever waited more than 10 minutes for an R script to finish executing, then you know the frustration of having to rerun it whenever your code changes. Plans help make life easier.

### Plans chop up the work into pieces.

Some targets may need an update, while others may not. In our [first example](#main), `make()` was smart enough to skip the data cleaning step and just rebuild the plot and report. Because plans chop up your work, it takes less thinking and less runtime to bring your results up to date.

### `drake` uses plans to schedule you work.

`make()` automatically learns the build order of your targets and [how to run them in parallel](#hpc). The magic is [_static code analysis_](http://adv-r.had.co.nz/Expressions.html#ast-funs), which allows `drake` to detect the dependencies of each target, command, and function.
...
Because of dependency relationships, row order does not matter once the plan is fully created. The following plan declares `file` before `plot`.

```{r smallplan__}
small_plan <- drake_plan(
  file = ggsave(file_out("plot.png"), plot),
...
The plan must include columns `target` and `command`, and you add other columns too.

```{r addcolplan}
cbind(small_plan, cpu = c(1, 2))
```

The `target()` function is a more convenient way to do this. Within `drake_plan()`, `target()` lets you create any custom column except `transform` and `group`, which are reserved keywords (see the next section).
...
`drake` version 7.0.0 will support new special syntax to create complicated plans from boilerplate code. The functionality will be officially considered experimental for the next few releases. To try it out before version 7.0.0 reaches [CRAN](http://cran.r-project.org), install the [current development version](https://github.com/ropensci/drake) from GitHub.
...
- **map()**: like `purrr::pmap()`. Iterates through the levels of grouping variables to create new targets. Grouping variables must be all the same length.
...
Writing out every single command by hand can get tiring.

```{r fulldrakeplanlongstats}
drake_plan(
  data = get_data(),
  analysis_0_1 = fit_model_x(data, mean = 0, sd = 1),
  analysis_1_1 = fit_model_x(data, mean = 1, sd = 1),
  analysis_2_1 = fit_model_x(data, mean = 2, sd = 1),
  analysis_3_1 = fit_model_x(data, mean = 3, sd = 1),
  analysis_4_1 = fit_model_x(data, mean = 4, sd = 1),
  analysis_5_1 = fit_model_x(data, mean = 5, sd = 1),
  analysis_6_1 = fit_model_x(data, mean = 6, sd = 1),
  analysis_7_1 = fit_model_x(data, mean = 7, sd = 1),
  analysis_8_1 = fit_model_x(data, mean = 8, sd = 1),
  analysis_9_1 = fit_model_x(data, mean = 9, sd = 1)
)
```
...
To get a model for each combination of `fit_model`, `mean_val` and `sd_val`, use `cross()`.
...
If you have multiple datasets, you can apply another transformation and then use `data` as a grouping variable for the analyses.
...
For really huge plans, you can define the grouping variables in advance and use the tidy evaluation operator `!!` to plug them in.
...
```
A reduction gathers targets into named lists. The following plan post-processes such a list into a data frame.
...
Supply grouping variables to `reduce()` to get a summary for each combination of levels.
...
Groupings for `reduce()` propagate downstream. Click and drag the nodes in the graph to unpack what is happening.
...
`drake` provides several older utility that increase the flexibility of workflow plan generation beyond `expand.grid()`.
