...
Similar to the `map()` and `pmap()` family in the [`purrr`](https://github.com/tidyverse/purrr) package, `map_plan()` generates a plan by mapping a function to a grid of arguments. For a practical example, see the [gross state product example in this manual](#gsp). But for now, let's stick to something simple.

Let's say we want find out which variables best explain fuel efficiency (in miles per gallon) in the `mtcars` dataset. We write a function that takes the names of two covariates and fits them to the response variable (`mpg`).
...
For our covariates, we consider all the variables in `mtcars` except `mpg`.

```{r map_plan_covariates}
covariates <- setdiff(colnames(mtcars), "mpg")
```

And we consider all possible pairs.
...
Each row of `args` represents a function call to `my_model_fit()` using the values in the row. `my_model_fit()` expects`x1` and `x2` to be character strings, so that is how we set them in `args`. But what about the `data` argument? In general, `data` could be the `mtcars` dataset or an unknown upstream target. For the "data" column, we need a list of symbols. The [`syms()`](https://rdrr.io/cran/rlang/man/sym.html) function from the [`rlang`](https://github.com/r-lib/rlang) package can help us.
...
For each row of `args`, we want to fit a model using the covariates in the row. Let's make a `drake` plan to do that.
...
The default target names are ugly, so we can supply our own names. You can either set an "id" column in `args` or set the `id` argument of `map_plan()`.
