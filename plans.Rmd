...
`drake` supports custom formats for large unwiedly targets. For example, the `"fst"` and `"fst_tbl"` formats use the [`fst`](https://github.com/fstpackage/fst) package to save `data.frame` and `tibble` targets faster. Simply enclose the command and the format together with the `target()` function.
...
- `"fst"`: for `data.frame` targets. The `"fst"` package must be installed, and the return value should be a `data.frame` (or at least coercible to one) and all non-data-frame attributes are discarded.
- `"fst_tbl"`: like `"fst"` format, but for `tibble` objects. Unsupported in `drake` version 7.9.0 and below. The `tibble` package must be installed, and all non-data-frame non-`tibble` attributes are discarded.
