...
volume_eroded <- volume_original - volume_current
volume_eroded
```
...
Functions are better than [imperative](https://en.wikipedia.org/wiki/Imperative_programming) scripts! They break down complex ideas into manageable pieces, and they gradually build up bigger pieces until an elegant solution materializes. This process of building up functions helps us think clearly, understand what we are doing, and explain our methods to others.

## Intro to plans

A `drake` plan is a data frame with columns named `target` and `command`. Each command is a concise [expression](http://adv-r.had.co.nz/Expressions.html) that calls our functions, and each target is the return value of the command. We create plans with the `drake_plan()` function.[^1]
...
Creating targets for the plan is more of an art than a science, and it requires personal judgement and context specific to your use case. Generally speaking, a good target is
...
4. A well-behaved R object compatible with `saveRDS()`. For example, data frames behave better than [R6 classes](https://github.com/ropensci/drake/issues/345) and [`xgboost` matrices](https://github.com/ropensci/drake/issues/1038).
...
`drake` has special functions to declare relationships between targets and files. [`file_in()`](https://docs.ropensci.org/drake/reference/file_in.html) is for input files, [`file_out()`](https://docs.ropensci.org/drake/reference/file_out.html) is for output files, and [`knitr_in()`](https://docs.ropensci.org/drake/reference/knitr_in.html) is for R Markdown reports and `knitr` source files. If you use one of these functions inline in the plan, it tells `drake` to rerun a target when a file changes.
...
An important caveat: `file_in()`, `file_out()`, and `knitr_in()` require you to mention file names *explicitly*. You cannot use a variable containing the name of a file.
