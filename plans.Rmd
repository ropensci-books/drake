...
plan1 <- drake_plan(
  ...
  summ_data = target(
    summary(metric),
    transform = combine(metric = list(), .by = data)
  ),
  # Same, but for each metric type.
  summ_metric = target(
    summary(metric),
    transform = combine(metric = list(), .by = metric_fun)
  )
)
...
In `combine()`, you can replace symbols in a command with collections of other targets. Unlike `map()` and `cross()`, `combine()` creates *aggregates* of existing grouping variables instead of the grouping variables themselves. Below, `combine(data = bind_rows(.id = "id")` tells `drake` to splice the levels of `data` into `bind_rows()` and then insert that `bind_rows()` call wherever there is a symbol called `data` in the command.
...
plan <- drake_plan(
  ...
  larger = target(
    arrange(data, sd) %>%
      head(n = 400),
    transform = combine(data = bind_rows(.id = "id"))
  )
)
...
You can combine multiple groups of targets together.
...
To splice different groups of targets together in the same call, use tags (more on that later).
...
plan <- drake_plan(
  ...
  data_group2 = target(
    pull_data(url),
    transform = map(
      url = c("example1.com", "example2.com"),
      .tag_out = data
    )
  ),
  larger = target(
    arrange(data, id),
    transform = combine(data = bind_rows())
  )
)
...
plan <- drake_plan(
  ...
  combined = target(
    arrange(data, sd) %>%
      head(n = 400),
    transform = combine(data = bind_rows(.id = "id"), .by = c(x, y))
  )
)
...
plan <- drake_plan(
  ...
  larger = target(
    data %>%
      arrange(sd) %>%
      head(n = 400),
    transform = combine(data = post_process(plan = ignore(plan)))
  ),
  trace = TRUE
)
...
plan <- drake_plan(
  ...
  summary = target(
    summarize(data),
    transform = combine(data = bind_rows(.id = "data"), .by = data_source)
  ),
  summ_num = target(
    summarize(data),
    transform = combine(data = bind_rows(.id = "data"), .by = n)
  )
)
