...
The grid comes from column-binding the arguments together, which means the lengths need to be conformable just as with `data.frame()`.
...
In most cases, grouping variables of unequal length give you sensible results.
...
But if other targets have `nrow`'s of their own, `drake_plan()` may not know what to do with them.

```{r drakemap4}
drake_plan(
  w = target(
    simulate_data(nrow, ncol),
    transform = map(nrow = c(3, 4))
  ),
  x = target(
    simulate_data(nrow, ncol),
    transform = map(nrow = c(1, 2))
...
The problems is that there are 4 values of `nrow` and only two `x_*` targets (and two `y_*` targets). Even if you explicitly supply `nrow` to the transformation, `map()` can only takes the first two values.
...
drake_plan(
  ...
  y = target(
    process_data(w, x, nrow),
    transform = map(w, x, nrow)
  ),
  trace = TRUE
)
```

So always inspect the plan before you run it with `make()`. Once you have a `drake_config()` object, `vis_drake_graph()` and `deps_target()` can help.
...
In `combine()`, you can gather lists of targets and insert them into commands. You must tell `combine()` which symbols you wish to replace.
...
`drake`'s default target names are way too long and ugly in some cases.
...
Fortunately, all three transformations have an optional `.id` argument. Set it to `FALSE` to severely shorten the target names.
...
For more suggestive names, you can select the grouping variables for the suffixes. In `map()` and `cross()`, all grouping variables are allowed except tags (defined later). In `combine()`, you can only use the symbols passed to `.by`.
