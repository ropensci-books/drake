...
`drake` supports three types of transformations: `map()`, `cross()`, and `combine()`. These are not actual functions, but you can treat them as functions when you use them. Each transformation takes after a function from the [Tidyverse](https://www.tidyverse.org/).
...
`combine()` aggregates other targets together into lists, which you can further process with `do.call()` and friends if you choose.
...
A grouping variable is an argument to `map()`, `cross()`, or `combine()` that identifies a subcollection of target names. Grouping variables can be either literals or symbols, and those symbols can be passed with names or as unnamed arguments. 
