...
A grouping variable is an argument to `map()`, `cross()`, or `combine()` that identifies a sub-collection of target names. Grouping variables can be either literals or symbols, and those symbols can be passed with names or as unnamed arguments. 
...
Symbol vectors passed with explicit argument names define new groupings of existing targets on the fly, and only the `map()` and `cross()` transformations can accept them. These new groupings carry over to subsequent transformations in the plan.
...
Again, `trace = TRUE` helps us understand what is happening. First, `val` is defined for the `z_*` targets and the `val` column is appended the plan. Then, when `drake_plan()` moves on to the `w_*` targets, the `z` column is already there.
...
Tags are special optional grouping variables that get added to the plan after a transformation is done. There are two types of tags:
