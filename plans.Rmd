...
Instead, use a transformation. As below, the tidy evaluation operator `!!` helps generate massive plans.
...
Behind the scenes, `drake_plan()` creates new columns as the transformation is happening. You can see those columns with `trace = TRUE`.
...
Those columns let you to chain transformations together to create complex pipelines. Here, the order of the transformations actually **does** matter. `drake_plan(transform = TRUE)` runs the transformations sequentially from top to bottom. However, after the transformations are done, the **targets** can be in any order for `make()`.
...
`drake` supports three types of transformations: `map()`, `cross()`, and `combine()`. They are not actual functions in `drake`, but you can treat them as functions when you pass them to `transform`. Each transformation is modeled after a function from the [Tidyverse](https://www.tidyverse.org/).
...
`map()` creates a new target for each row in a grid. The grid comes from column-binding the arguments, so the lengths need to be conformal just like with `data.frame()`.
...
`combine()` aggregates other targets together into lists, which you can further process with `do.call()`.
...
You can aggregate separately for each combination of argument levels.
...
With the exception of `.tag_in` and `.tag_out` (more on that later) a grouping variables is an argument to `map()`, `cross()`, or `combine()`. Grouping variables can be literals or symbols, and grouping symbols can be passed as named or unnamed arguments. 

#### Literal arguments

It is usually straightforward to supply named literals to transformations. Below, the grouping variable is named in the sense that `y` is given explicitly as an argument to `map()`. One does not simply write `map(c(1, 2))`.

```{r namedlittrans}
drake_plan(x = target(sqrt(y), transform = map(y = c(1, 2))))
```

If you try to supply integer sequences, you may notice some rows are missing.

```{r namedlittrans2}
drake_plan(x = target(sqrt(y), transform = map(y = 1:3)))
```

Tidy evaluation and `as.numeric()` really help here.
...
Only valid for the `map()` and `cross()` transformations. Symbols passed as named arguments define new groupings of existing targets on the fly. Transformations in subsequent rows can refer back to those groupings with unnamed symbols.
...
To see why, run `drake_plan()` with `trace = TRUE`.
...
To generate long symbol lists, use the `syms()` function from the `rlang` package. Remember to use the tidy evaluation operator `!!`.
...
Tags are special grouping variables you add after the transformation is done, and they decrease the typing you need for writing intricate plans. There are two types of tags:
...
Subsequent transformations can add to existing tags and/or use tags as grouping variables.
