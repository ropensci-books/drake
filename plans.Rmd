...
Those columns let you to chain transformations together to create complex pipelines.
...
plan <- drake_plan(
  ...
  summ_data = target(summary(metric), transform = reduce(data)),
  # Same, but for each metric type.
  summ_metric = target(summary(metric), transform = reduce(metric_fun))
)
...
`drake` supports three types of transformations: `map()`, `cross()`, and `reduce()`. They are not actual functions in `drake`, but you can treat them as functions when you pass them to `transform`. Each transformation is modeled after a function from the [Tidyverse](https://www.tidyverse.org/).

| `drake`    | Tidyverse                  |
|------------|----------------------------|
| `map()`    | `pmap()` from `purrr`      |
| `cross()`  | `crossing()` from `tidyr`  |
| `reduce()` | `summarize()` from `dplyr` |
...
- **map()**: like `purrr::pmap()`. Creates a new target for each index along your grouping variables. Grouping variables must have conformable lengths just like columns in `data.frame()`.
...
Groupings for `reduce()` propagate downstream. Click and drag the nodes in the graph to see the results of the transformations.
