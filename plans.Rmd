...
A `drake` plan is a data frame with columns `"target"` and `"command"`. Each target is the name of an R object produced in your workflow, and each command is the R code that produces it. In this sense, the `drake` plan is a tidy representation of the steps of your data analysis project. You can create plans any way you like, but the `drake_plan()` function is usually the most convenient way. Recall the plan from our [previous example](#main):
...
Creating the plan does not actually run the workflow. To build the targets, we need to run `make()` on the plan just as we would [`source()`](https://www.rdocumentation.org/packages/base/versions/3.5.2/topics/source) an R script.
...
If you have ever waited more than 10 minutes for an R script to finish running, then you know the frustration of having to rerun it whenever you go back and change your code. Plans help make life easier.

### Plans break up the work into skippable steps.

As we saw in our [first example](#main), `make()` skips steps of your workflow that are already up to date. In order to skip those steps, we need to define them up front, which is exactly what `drake` plans do.

### `drake` uses plans to schedule you work.

You need to know how to build each target, but because of plans, you do not need to think too hard about how the targets fit together. `drake` analyzes the plan's commands to figure out which targets to run at which times during `make()`. You can check what `drake` is thinking with the [`vis_drake_graph()` function](#visuals).

The magic is in _static code analysis_, where `drake` looks at the dependencies of your commands without actually running them (ref: [_Advanced R_](http://adv-r.had.co.nz/Expressions.html#ast-funs), [`CodeDepends`](https://github.com/duncantl/CodeDepends)).
...
The following plan declares `file` before `plot`.
...
The `target()` function is a more convenient way to do this.
...
## Generating large plans the new way

Coming soon...

## Generating large plans the old way
