...
### Transformations: a high level overview
...
Transformations reduce typing. As shown below, the tidy evaluation operator `!!` is particularly helpful.
...
Those columns let you to chain transformations together to create complex pipelines. Here, the order of the transformations actually **does** matter because `drake_plan(transform = TRUE)` runs the transformations sequentially from top to bottom. However, after the transformations are all done, the **targets** can sit anywhere in the plan for `make()`.
...
plan <- drake_plan(
  ...
  summ_data = target(summary(metric), transform = combine(data)),
  # Same, but for each metric type.
  summ_metric = target(summary(metric), transform = combine(metric_fun))
)
...
`drake` supports three types of transformations: `map()`, `cross()`, and `combine()`. They are not actual functions in `drake`, but you can treat them as functions when you use them. Each transformation takes after a function from the [Tidyverse](https://www.tidyverse.org/).
...
`map()` creates a new target for each row in a grid. The grid comes from column-binding the arguments together, which means the lengths need to be conformable just like with `data.frame()`.
...
`combine()` aggregates other targets together into named lists, which you can further process with `do.call()`.
...
As with `group_by()` from `dplyr`, you can aggregate separately for each combination of levels of the arguments.
...
If you need the aggregated list elements to have names, set `.use_names` to `TRUE`. The default is `FALSE` because names usually make the commands quite long.

```{r transform7combinenamed}
plan <- drake_plan(
  data = target(
    sim_data(mean = x, sd = y, skew = z),
    transform = cross(x = c(1, 2), y = c(3, 4), z = c(5, 6))
  ...
  target(
    do.call(rbind, data),
    transform = combine(x, y, .use_names = TRUE)
  )
)
...
With the exception of `.tag_in` and `.tag_out` (more on that later) a grouping variables is an argument to `map()`, `cross()`, or `combine()`. Grouping variables can be literals or symbols, and grouping symbols can be passed with names or as unnamed arguments. 

#### Literal arguments

When you pass literals, you must use explicit argument names. One does not simply write `map(c(1, 2))`.
...
Character vectors usually work without a hitch.

```{r namedlittrans4}
drake_plan(x = target(get_data(y), transform = map(y = c("a", "b"))))
```
...
Only valid for the `map()` and `cross()` transformations. Symbols passed as named arguments define new groupings of existing targets on the fly. Transformations in subsequent rows can refer back to those groupings using unnamed symbol arguments.
...
drake_plan(
  ...
  w = target(z + 1, transform = map(val))
)
```

`drake_plan(trace = TRUE)` gives us a hint as to why. First, `val` is defined for the `z_*` targets and the `val` column is appended the plan. So when `drake_plan()` moves on to the `w_*` targets, the `z` column is already there.
...
To generate long symbol lists, use the `syms()` function from the `rlang` package. Remember to use the tidy evaluation operator `!!` inside the transformation.
...
Tags are special optional grouping variables that get added to the plan after a transformation is done, and they decrease the typing you need for writing complicated plans. There are two types of tags:

1. In-tags, which contain the target name you start with, and
2. Out-tags, which contain the target names generated by transformations.
...
Subsequent transformations can add to existing tags, and more importantly, use tags as grouping variables.
