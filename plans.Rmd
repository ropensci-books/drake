...
In the following example, we want to know how well each pair covariates in the [`mtcars` dataset](https://stat.ethz.ch/R-manual/R-devel/library/datasets/html/mtcars.html) can predict fuel efficiency (miles per gallon). We will try multiple pairs of covariates using the same statistical analysis, so it is a great time for `drake`-flavored functional programming with `map_plan()`.
...
We now have a plan, but it has problems.
...
2. The default argument names are ugly, so we should add a new `"id"` column to `args` (or select one with the `id` argument of `map_plan()`).
