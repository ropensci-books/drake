...
1. Initial discard: before building the target, discard some other targets from the R session. (Note: the actual data does not relinquish its memory until `gc()` is called.)
2. Initial load: before building the target, check the target's dependencies and load any that are missing.
...
However, you can also use it to load individual pieces of a large file. The trick is to use `split()` to break up an index set instead of the data itself. In the sketch below, `get_number_of_rows()` and `read_selected_rows()` are user-defined functions, and `%>%` is the `magrittr` pipe.
