...
In dynamic branching, the trace is a way to document how we generate the sub-targets. We can use it to remember which continent belongs to which model. `map()`, `cross()`, and `combine()` each support a `.trace` argument for this purpose.
