...
In dynamic branching, tracing keeps track of the sub-dependencies that correspond to individual the sub-targets. We can use tracing to remember which continent belongs to which model. `map()`, `cross()`, and `combine()` each support a `.trace` argument for this purpose.
...
The trace also helps us create grouping variables for dynamic targets downstream of `cross()` and `combine()`. Below, we fit multiple models to multiple countries and aggregate on country. `get_trace()` gets grouping variables easily.
