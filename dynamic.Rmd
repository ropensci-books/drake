...
If you put multiple grouping variables in the same `map()` (e.g. `static_numbers` and `static_letters` for `dynamic_lowercase` above) they must have the same `NROW()`.

```{r}
NROW(readd(static_numbers))

NROW(readd(static_letters))
```

For vectors and simple lists, `NROW()` is just the length. For array-like objects (anything with a non-null `dim()`) `NROW()` is the size of first dimension. For data frames, this is the number of rows. In other words, dynamic branching always iterates over the *rows* of data frames, not the columns.[^1]
