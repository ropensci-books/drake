...
This makes RDS caches difficult to share with collaborators and put under version control. For the sake of portability, you may wish to convert all this data into a single SQLite database file.

```{r converttosqlite}
# Get the old RDS cache.
cache <- get_cache()

# Create and connect to a single database file.
mydb <- DBI::dbConnect(RSQLite::SQLite(), "database-file.sqlite")

# Initialize a storr cache in that database file.
portable <- storr::storr_dbi("datatable", "keystable", mydb)

# Transfer the data from the RDS cache to the database file.
portable$import(cache) # Alternative: cache$export(portable)
head(portable$list())
```

Now, you can more easily share the single `database-file.sqlite` with your collaborators, who can then convert it to their own RDS caches if needed.

```{r portabledisconnect, echo = FALSE}
portable$destroy()
```
...
It is possible to work entirely with database caches, with no need to convert to and from the RDS format.
...
Hash functions vary. Some generate larger hash keys, some are slower to compute, and others are more prone to "collisions" (where two different data objects are given the same hash key). The `digest` package supports a variety of hash algorithms.
