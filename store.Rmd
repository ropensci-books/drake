...
1. Be careful about parallel computing. For example the `storr::storr_dbi()` cache is not thread-safe. To use these exotic cache types in parallelized `make()`s, select `caching = "master"` and set `parallelism` equal to `"clustermq"`, `"clustermq_staged"`, or `"future"`. In addition, make sure imports are not parallelized (i.e. use something like `make(jobs = c(imports = 1, targets = 4))`.
