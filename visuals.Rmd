...
text_drake_graph(config, nchar = 3)
```
...
As previously mentioned, you can group on any column in `drake_graph_info(config)$nodes`. Let's return to the `mtcars` project for demonstration.
...
*(Supported in drake > 7.7.0, non-inclusive)*
...
treats the values in the `on_select_col` as hyperlinks.
