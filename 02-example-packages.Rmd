...
triggers to force `recent` to always build. For more on triggers, see the [vignette on debugging and testing](https://github.com/ropensci/drake/blob/master/vignettes/debug.Rmd). Instead of triggers, we could have just made `recent` a global variable like `package_list` instead of a formal target in `data_plan`.
...
<iframe
src = "https://ropensci.github.io/drake/images/packages.html"
...
When you rely on data from the internet, you should trigger a new download when the data change remotely. [This section of the best practices guide](https://github.com/ropensci/drake/blob/master/vignettes/best-practices.Rmd#remote-data-sources) explains how to automatically refresh the data when the online timestamp changes.
