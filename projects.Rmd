...
1. Supply the namespace of your package to the `envir` argument of `make()` and friends (e.g. `make(envir = getNamespace("yourPackage")` so `drake` can watch you package's functions for changes and rebuild downstream targets accordingly.
