...
The quickest workaround is to restart R and `source()` your setup scripts all over again. However, a better solution is to use [`r_make()`](https://ropensci.github.io/drake/reference/r_make.html) and friends. [`r_make()`](https://ropensci.github.io/drake/reference/r_make.html) runs `make()` in a new transient R session so that accidental changes to your interactive environment do not break your workflow.
