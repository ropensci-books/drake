...
On Mac and Linux, you can do this by opening a terminal and entering `R CMD BATCH make.R`. On Windows, restart your R session and call `source("make.R")` in the R console.
...
If you keep an interactive session running too long, you might accidentally change in-memory dependencies, such as `create_plot()` from the previous example. That could cause targets such as `hist` and `report` to fall out of date without warning. If this happens, the quickest fix is to restart R and `source()` your setup scripts all over again. However, a better solution is to use [`r_make()`](https://ropensci.github.io/drake/reference/r_make.html) and friends. [`r_make()`](https://ropensci.github.io/drake/reference/r_make.html) runs `make()` in a new transient R session so that accidental changes to your interactive environment do not break your workflow.

### Usage

To use [`r_make()`](https://ropensci.github.io/drake/reference/r_make.html), you need a configuration R script. Unless you supply a custom file path (e.g. `r_make(source = "your_file.R")` or `options(drake_source = "your_file.R")`) `drake` assumes this configuration script is called `_drake.R`. (So the file name really *is* magical in this case). The suggested file structure becomes:
...
Like our previous `make.R` script, `_drake.R` runs all our pre-`make()` setup steps. But this time, rather than calling `make()`, it ends with a call to `drake_config()`.
...
```{r exconfig, eval = FALSE}
config <- drake_config(plan, parallelism = "clustermq", jobs = 2, verbose = 6)
make(config = config)
```
