...
The `load_mtcars_example()` function loads the [mtcars example](https://github.com/ropensci/drake/tree/master/inst/examples/mtcars) from `drake_example("mtcars")` right into your workspace. The workflow plan data frame, workspace, and import files are set up for you. Only `make(my_plan)` is left to you.
