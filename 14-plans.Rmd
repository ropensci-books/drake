...
This chapter is all about workflow plan data frames and how to create, modify, and execute them.

## What is a workflow plan data frame?

### A manifesto of targets

The purpose of the plan to declare the data objects and files you are going to produce when you run your project later on. These object/file outputs are called *targets*.

```{r what-is-a-plan}
library(drake)
plan <- drake_plan(
  average = mean(c(normal, uniform)),
  normal  = rnorm(50),
  uniform = runif(10)
)
plan
...
Creating the plan does not actually build our targets `uniform` and `normal`. For that, we have to use `make()`.

```{r what-is-a-plan-make}
make(plan)
```

The targets live in a cache (`.drake/` folder), and you can access them with the `loadd()` and `readd()` functions.


```{r what-is-a-plan-readd}
mean(readd(uniform))
```
