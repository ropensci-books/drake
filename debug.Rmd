...
With `deps_targets()`, you can see the dependencies that `drake` has already detected for your targets and imports.

```{r checkdepsdebug2}
deps_targets("simulate", config)

deps_targets("small", config)

deps_targets("report", config)
```
...
Then, we forget to run `make()` again, and we leave the the project for several months. When we come back, all our targets are suddenly out of date, and we have no idea why.

```{r whyoutdated}
outdated(config)
```

The `dependency_profile()` function tells us which first-order dependencies of targets in `my_plan` changed since the last time we ran `make()`. We care most about our final report, so let's start there. Why is our report out of date?

```{r depprofile1}
dependency_profile("report", config)
```

It looks like none of the direct dependencies have different values since last time, so we need to look further upstream.

```{r dependencyprofile}
dependency_profile(coef_regression2_small, config)

dependency_profile(regression2_small, config)

dependency_profile(small, config)
```

Aha! So either

1. The function body of `simulate()` changed, or
2. One of the objects nested in `simulate()` changed. We need to dive into `simulate()`, but `dependency_profile()` is not useful for imports, so we continue on foot.

```{r inspectsimulate}
print(simulate) # Looks the same as last time.

deps_targets("simulate", config) # Let's try random_rows.

print(random_rows) # Now that looks different...
```

At some point, we added some error checking to `random_rows()`. With a fresh perspective, we now realize that this error checking is unnecessary. So we revert to the previous version of `random_rows()`. Now, all our targets are up to date again.
